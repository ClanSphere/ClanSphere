function cs_ajax_setcontent (request, id, onfinish, setanch) {
  ...
  document.getElementById(id).innerHTML = (!mod_rewrite) ? response.replace(/href=\"([a-z\/\.\-\_]*?)\?mod=(\w.+?)\"/g,"href=\"#mod=$2\"") :
    response.replace(/href=\"\/[a-z\/\.\-\_]*?(content|navlists)\/(\w.+?)\"/g,"href=\"#$2\"");
  if (id == 'content') {
    request_cont = 0;
    if (setanch) {
...
function initializeAJAX(modrewrite, navinterval) {
  ...
  if (modrewrite) {
    mod_rewrite = 1;
    cont = cont.replace(/href=\"\/[a-z\/\.\-\_]*?(index|debug|board)\/(\w.+)\"/g,"href=\"#$2\"");
  } else
    cont = cont.replace(/href=\"([a-z\/\.\-\_]*)\?mod=(\w.+?)\"/g,"href=\"#mod=$2\"");
  ...
  for(i = 0; i <forms.length; i++) {
    ...
    forms[i].onsubmit = function() {
      ...
      document.getElementById('content').innerHTML += '<img src="uploads/ajax/loading.gif" id="ajax_loading" alt="Loading.." />';
        ...
        cs_ajax_getcontent(this.action.replace(/([a-z\/\.\-\_\:]*)\?mod\=(\w.+)/g,"content.php?mod=$2"),'content',  "cloaded('"+anch+"')" , form_data, 1);
      } else {
