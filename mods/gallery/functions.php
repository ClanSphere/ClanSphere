  ...
  if(!empty($array)) {
    ...
    if($list_sort == 0) {
      for($run=0; $run < $loop; $run++) {
        ...
        if($run == '0') {
          $move = 0;
          $temp_array[$move][$run]['gallery_id'] = $array[$run]['gallery_id'];
      ...
      if(isset($temp_array)) {
        ...
        for($run=0; $run < $loop; $run++) {
          ...
          for($loop_2; $loop_2 >= 0; $loop_2--) {
            ...
            $temp_array2[$run_to]['gallery_id'] = $temp_array[$run][$run_to]['gallery_id'];
            $run_to++;
          }
        }
        $array = cs_array_sort($array,SORT_DESC,'gallery_id');
        $temp_array2 = cs_array_sort($temp_array2,SORT_DESC,'gallery_id');
        $loop = count($array);
        for($run=0; $run < $loop; $run++) {
...
function cs_box($array,$run) {
  if(!empty($array)) {
    ...
    global $account;
    global $cs_lang;
    if($array[$run]['gallery_status'] == 1) {
      $data['status']['icon'] = "symbols/clansphere/green.gif";
    } else {
      $data['status']['icon'] = "symbols/clansphere/red.gif";
    }
    $pic = cs_secure($array[$run]['gallery_name']);
    $img_size = getimagesize('uploads/gallery/thumbs/Thumb_' . $pic);
    $img_w_h = $img_size[0] / $img_size[1];
    $img_height = 40;
    $img_width = $img_height * $img_w_h;
    $pic = cs_secure($array[$run]['gallery_id']);
    $pic = cs_html_img('mods/gallery/image.php?thumb=' . $pic,$img_height,$options['thumbs']);
    $link = 'folders_id=' . $array[$run]['folders_id'] . '&amp;where=' . cs_secure($array[$run]['gallery_id']);
    $data['link']['img'] = cs_link($pic,'gallery','com_view',$link);
    $data['link']['titel'] = cs_link(cs_secure($array[$run]['gallery_titel']),'gallery','com_view',$link);
    $link = 'id=' . $account['users_id'] . '&amp;folders_id=' . $array[$run]['folders_id'];
    $data['link']['folders_name'] = cs_link(cs_secure($array[$run]['folders_name']),'gallery','list',$link);
    $data['data']['date'] = cs_new_date('unix',$array[$run]['gallery_time'],1);
    $img_edit = cs_icon('edit',22,$cs_lang['edit']);
    $data['link']['edit'] = cs_link($img_edit,'gallery','picture_edit','id=' . $array[$run]['gallery_id'],0,$cs_lang['edit']);
    $img_del = cs_icon('editdelete',22,$cs_lang['remove']);
    $data['link']['remove'] = cs_link($img_del,'gallery','remove','id=' . $array[$run]['gallery_id'],0,$cs_lang['remove']);
    return $data;
  }
}
  ...
  if (!empty($array)) {
    foreach ($array AS $folder) {
      ...
      if (!empty($folder[0])) {
        ...
        $data['link']['edit'] = cs_link($img_edit,'gallery','folders_edit','id=' . $folder['folders_id'],0,$cs_lang['edit']);
        $data['link']['remove'] = cs_link($img_del,'gallery','folders_remove','id=' . $folder['folders_id'],0,$cs_lang['remove']);
...
function make_folders_select($name,$select,$id = 0,$mod = 0,$create = 1, $folders_id = 0) {
  ...
  $where = "folders_mod='" . $mod . "'";
  $order = 'folders_id ASC';
  $array = cs_sql_select(__FILE__,'folders',$sql_select,$where,$order,0,0);
  $array = make_folders_array($array);
  $var = cs_html_select(1,$name);
  $sel = 0;
  $var .= cs_html_option('----','0',$sel);
  $var .= make_folders_options($array,0,$select, $folders_id);
  $var .= cs_html_select(0);
  if (!empty($create)) $var .= ' - ' . cs_html_input('folders_name','','text',80,20);
  return $var;
}
