...
$data['head']['pages'] = cs_pages('gbook','center',$gbook_count,$start,0,$sort);
...
$select .= 'gbk.gbook_email AS gbook_email, ';
$select .= 'usr.users_nick AS users_nick, ';
$select .= 'usr.users_email AS users_email';
$cs_gbook = cs_sql_select(__FILE__,$from,$select,$where,$order,$start,$account['users_limit']);
$gbook_loop = count($cs_gbook);

$data['sort']['email'] = cs_sort('gbook','center',$start,0,3,$sort);
$data['sort']['time'] = cs_sort('gbook','center',$start,0,1,$sort);
