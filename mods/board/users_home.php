...
if (!empty($data['threads'])) {
  ...
  for ($run = 0; $run < $count_threads; $run++) {
    ...
    $data['threads'][$run]['users_nick'] = !empty($data['threads'][$run]['users_nick']) ? cs_html_br(1) . $cs_lang['from'] . ' ' . cs_user($data['threads'][$run]['users_id'],$data['threads'][$run]['users_nick'],$data['threads'][$run]['users_active']) : '';
  }
