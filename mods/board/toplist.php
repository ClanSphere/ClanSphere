...
$cells = 'usr.users_id AS users_id, usr.users_nick AS users_nick, usr.users_active AS users_active, COUNT(DISTINCT cms.comments_id) + COUNT(DISTINCT thr.threads_id) AS comments'; 
...
for($run = 0; $run < $count_users; $run++) {
  if(!empty($cs_users[$run]['users_id']) AND $cs_users[$run]['users_active'] > 0) {
    $x++;
