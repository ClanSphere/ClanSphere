...
$tables = 'comments cms LEFT JOIN {pre}_users usr ON cms.users_id = usr.users_id AND usr.users_active = "1" AND usr.users_delete = "0" LEFT JOIN {pre}_threads thr ON thr.users_id = usr.users_id';
...
$select = cs_sql_select(__FILE__,$tables,$cells,"comments_mod = 'board' GROUP BY usr.users_id, usr.users_nick",'comments DESC, usr.users_nick',0,3);
$d['stats']['toplist'] = '';
if (!empty($select)) {
	foreach ($select AS $user) {
	  if (!empty($user['users_id']))
		  $d['stats']['toplist'] .= cs_user($user['users_id'],$user['users_nick']) . ' (' . $user['comments'] . ' ' . $cs_lang['posts'] . '), ';
	}
