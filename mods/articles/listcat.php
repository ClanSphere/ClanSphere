...
if (empty($category)) {
	...
	for($run=0; $run<$articles_loop; $run++) {
		...
		$cs_articles_user = cs_sql_select(__FILE__,'users','users_nick, users_active',"users_id = '" . $cs_articles[$run]['users_id'] . "'");
		$cs_articles[$run]['articles_user'] = cs_user($cs_articles[$run]['users_id'],$cs_articles_user['users_nick'],$cs_articles_user['users_active']);
