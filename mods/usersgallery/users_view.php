...
$select .= $from . '_vote, ' . $from . '_count, categories_id';
$where = "categories_id = '" . $cat_id . "' AND usersgallery_status = '1' AND usersgallery_access <= '" . $access_id . "'";
...
else
{
	...
	$where = "categories_mod='usersgallery_" . $id . "' AND categories_id = '" . $cat_id . "'";
	...
	$head = cs_link($cs_lang['mod'],'usersgallery','users','id='. $id) .' - ';
	$head .= cs_link($cs_cat['categories_name'],'usersgallery','users','cat_id='. $cs_cat['categories_id'] .'&amp;id='. $id);
	...
	if(!empty($move)) {
		$back_1 = $move-1;
	//	$back = cs_html_img("mods/gallery/image.php?usersthumb=" . $cs_gallery[$back_1]['usersgallery_id']);
	//	echo cs_html_div(1,'float:left');
	//	echo cs_html_img('symbols/gallery/picture_prev.png',0,0,0,$cs_lang['back']);
	//	echo cs_link($back,'usersgallery','users_view','cat_id='. $cat_id .'&amp;move='. $back_1 .'&amp;id=' . $id);
	//	echo cs_html_div(0);
	}
	$forward_1 = $move+1;
	...
	//	echo cs_link($forward,'usersgallery','users_view','cat_id='. $cat_id .'&amp;move='. $forward_1 .'&amp;id=' . $id);
	//	echo cs_html_div(0);
	}
	echo cs_html_roco(0);

	echo cs_html_roco(1,'centerc',0,2);
	echo cs_link($cs_lap,'usersgallery','com_view','where=' . $cs_gallery[$move]['usersgallery_id'] .'&amp;id='. $id);
	echo cs_html_roco(0);
	...
	if(!empty($move)) {
		...
		$back_1 = $move-1;
		echo cs_link($back,'usersgallery','users_view','cat_id='. $cat_id .'&amp;move='. $back_1 .'&amp;more=' . $detail .'&amp;id=' . $id);
	}
	echo ' ' . $forward_1 . '/' . $gallery_loop . ' ';
	if($forward_1 < $gallery_loop) {
		$forward = cs_icon('forward',22,$cs_lang['forword']);
		echo cs_link($forward,'usersgallery','users_view','cat_id='. $cat_id .'&amp;move='. $forward_1 .'&amp;more=' . $detail .'&amp;id=' . $id);
	}
	echo cs_html_roco(2,'centerb',0,0,'50%');
	$view_bottom = cs_icon('view_bottom',22,$cs_lang['more']);
	if($detail == '1') {
		echo cs_link($view_bottom,'usersgallery','users_view','cat_id='. $cat_id .'&amp;move='. $move .'&amp;more=0&amp;id=' . $id);
	} else {
		echo cs_link($view_bottom,'usersgallery','users_view','cat_id='. $cat_id .'&amp;move='. $move .'&amp;more=1&amp;id=' . $id);
	}
	//$com = cs_icon('kopete',22,$cs_lang['comment']);
	//echo cs_link($com,'usersgallery','com_view','where=' . $cs_gallery[$move]['usersgallery_id'] .'&amp;id='. $id);
	echo cs_html_roco(0);
	if($detail == '1')
	{
		...
		if(!empty($check_vote))
		{
			...
			{
				echo cs_html_roco(2,'leftc');
				echo cs_html_form(1,'usersgallery','usersgallery','users_view&amp;cat_id='. $cat_id .'&amp;move='. $move .'&amp;more=1&amp;id=' . $id);
	...
	if($detail == '1')
	{
		...
		{
			echo cs_html_br(1);
			echo cs_comments_view($usersgallery_id,'usersgallery','users_view',$com_loop);
		}
		if(empty($cs_gallery[$move]['usersgallery_close']))
		{
			echo cs_html_br(1);
			echo cs_comments_add($usersgallery_id,'usersgallery');
		}
	}
}
?>