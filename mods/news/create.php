...
if(isset($_POST['preview']) and empty($error)) {
  ...
  $data['news']['preview_news_text'] = cs_secure($cs_news['news_text'],1,1,1,1,1);

  $data['if']['readmore'] = false;

  if(!empty($cs_news['news_readmore'])) {
    $data['if']['readmore'] = true;
    $data['news']['preview_news_readmore'] = cs_secure($cs_news['news_readmore'],1,1,1,1,1) . cs_html_br(2);
  }
