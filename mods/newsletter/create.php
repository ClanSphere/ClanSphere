...
if(!empty($error) OR !isset($_POST['submit'])) {
  ...
  foreach($usergroups AS $value) {
    $dp .= cs_html_option('&nbsp;&nbsp;&nbsp;' .$value['access_name'],'2?' .$value['access_id']);
  }
  ...
  foreach($squads AS $squad) {
    $dp .= cs_html_option('&nbsp;&nbsp;&nbsp;' .$squad['squads_name'],'3?' .$squad['squads_id']);
  }   
  ...
  foreach($clans AS $clan) {
    $dp .= cs_html_option('&nbsp;&nbsp;&nbsp;' .$clan['clans_name'],'4?' .$clan['clans_id']);
  }
