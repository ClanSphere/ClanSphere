<?php

// Modul Nombre : Translation in Spanish
// CS Version : ClanSphere 2008 - SP1
// Team :  quemao-designz.de - www.quemao-designz.de
// Author : quema0
// Author eMail : quemao@quemao-designz.de
// $Id$

$cs_lang['mod']	= 'Links';
$cs_lang['text']	= 'Modulo de Links';

$cs_lang['head']= 'Lista';
$cs_lang['body_list']= 'Hay %s Links en Total';

$cs_lang['all'] = 'Total : ';
$cs_lang['url'] = 'URL';
$cs_lang['cat'] = 'Categoria';
$cs_lang['stat'] = 'Status';
$cs_lang['new_link'] = 'Nuevo Link';


$cs_lang['online'] = '[color=lime]online[/color]';
$cs_lang['offline'] = '[color=red]offline[/color]';

$cs_lang['head_remove']	= 'Remover';
$cs_lang['newicon'] = 'Nuevo Baner';
$cs_lang['delicon'] = 'Remover Baner';



$cs_lang['mod1']	= 'Patrocinadores y Socios';
$cs_lang['all1'] = 'Hay %s Patrocinadores / Socios en Total .';

$cs_lang['info'] = 'Infos';
$cs_lang['head1'] = 'Detalles';   

$cs_lang['head2'] = 'Crear';
$cs_lang['body_create'] =  'Por Favor rellene todas las casillas con * ';
$cs_lang['error_occurred'] = 'No han sido Rellenadas todas las casillas necesitadas.';
$cs_lang['error_count'] = ' casillas necesitadas faltan';

$cs_lang['categories'] = 'Categoria';
$cs_lang['status'] = 'Status';
$cs_lang['icon'] = 'Baner'; 

$cs_lang['extension'] = 'Avanzado';
$cs_lang['sponsorbox'] = 'Inscribir como Patrocinador / Socio';

$cs_lang['ext_error'] = '- La extension que usa no es Valida';
$cs_lang['too_wide'] = '- Baner es muy Ancho';
$cs_lang['too_high'] = '- Baner es muy Alto';
$cs_lang['too_big'] = '- La Capacidad del Archivo es muy grande';

$cs_lang['pic_infos'] = 'Infobox';
$cs_lang['max_width'] = 'Anchura Maximal : ';
$cs_lang['max_height'] = 'Altura Maximal : ';
$cs_lang['max_size'] = 'Capacidad Maximal :  ';
$cs_lang['filetypes'] = 'Formados de Archivos : ';

$cs_lang['cat_manage'] = 'Administración';
$cs_lang['cat_create'] = 'Crear';

$cs_lang['online1'] = 'online';
$cs_lang['offline1'] = 'offline';

$cs_lang['success_noup'] = 'El Link ha sido creado, pero la Imagen no ha Podido ser cargada';

?>