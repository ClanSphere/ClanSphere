<?php


// Modul Nombre : Translation in Spanish
// CS Version : ClanSphere 2008 - SP1
// Team :  quemao-designz.de - www.quemao-designz.de
// Author : quema0
// Author eMail : quemao@quemao-designz.de
// $Id$

// info.php
$cs_lang['mod_name']  = 'Articulos';
$cs_lang['mod_text']  = 'El Modul de Articulos';
$cs_lang['mod']  = 'Articulos';

// create.php
$cs_lang['head_create'] = 'Incribir';
$cs_lang['body_create'] =  'Por favor rellene las casillas con *';
$cs_lang['error_occurred'] = 'No han sido rellenadas todas las casillas necesitadas';
$cs_lang['error_count'] = 'casillas necesitadas faltan';
$cs_lang['no_cat'] = '- Tiene que escoger una Categoria';
$cs_lang['no_headline'] = '- La cabecera no puede estar vacia';
$cs_lang['headline'] = 'Cabezera';
$cs_lang['categories'] = 'Categoria';
$cs_lang['pagebreak'] = 'nueva Pagina';
$cs_lang['more'] = 'Avanzado';
$cs_lang['nocom'] = 'Bloquear comentarios';
$cs_lang['nav'] = 'Incribir en la Lista de Navigacion'; 
$cs_lang['fornext'] = 'Esconder Paginas';
$cs_lang['cat_manage'] = 'Administración';
$cs_lang['cat_create'] = 'Crear';
$cs_lang['sitelink'] = 'Link de la pagina';

// edit.php
$cs_lang['head_edit'] = 'Editar';
$cs_lang['body_edit'] =  'Por favor rellene las casillas con *';
$cs_lang['edit_done'] = 'Operacion Valida';

// list.php
$cs_lang['head_list']  = 'Lista';
$cs_lang['body_list']  = 'Escoja una Categoria';
$cs_lang['count']  = 'Cantidad ';

//listcat.php
$cs_lang['all'] = 'Total: ';
$cs_lang['release'] = 'Publicación';
$cs_lang['user'] = 'Autor';
$cs_lang['views'] = 'Leido';
$cs_lang['xtimes'] = 'veces';

// manage.php
$cs_lang['head_manage']  = 'Administración';
$cs_lang['new_article'] = 'Nuevo Articulo';

// remove.php
$cs_lang['head_remove']  = 'Remover';
$cs_lang['body_remove']    = 'Remover Informacion Nr. %s de verdad ?';

// view.php
$cs_lang['from'] = 'Incribido por';
$cs_lang['at'] = 'de ';
$cs_lang['page'] = 'Pagina';
$cs_lang['pages'] = 'Paginas';

// com_create.php
$cs_lang['not_able']  = '- Los Comentarios han sido bloqueados.';

$cs_lang['place']      = 'Lugar :';
$cs_lang['posts']      = 'Aportes :';
$cs_lang['text'] = 'Texto';

// com_edit.php
$cs_lang['head_com_edit']  = 'Editar Comentarios';
$cs_lang['body_com_edit']  = 'Por favor Informar sobre editaciones del Texto';

// com_remove.php
$cs_lang['head_com_remove']  = 'Remover Comentarios';

$cs_lang['new_date'] = 'Actualizar la Fecha';

?>