<?php

// Modul Nombre : Translation in Spanish
// CS Version : ClanSphere 2008 - SP1
// Team :  quemao-designz.de - www.quemao-designz.de
// Author : quema0
// Author eMail : quemao@quemao-designz.de
// $Id$

$cs_lang['mod_name']	= 'Usuario';
$cs_lang['users'] = 'Usuario';
$cs_lang['mod_text']	= 'Administración de las Cuentas de Usuarios';

$cs_lang['wizard'] = 'Asistente';
$cs_lang['task_done'] = 'Marcar como Acabado';
$cs_lang['readtime'] = 'Marcar temas del foro no leidos despues de';
$cs_lang['days'] = 'dias';
$cs_lang['automatic'] = 'Automaticamente';
$cs_lang['no_readtime'] = 'Escriba la cantidad de dias';

// Navlist
$cs_lang['admin'] = 'Administración';
$cs_lang['cookie'] = 'Aceptar Cookie';
$cs_lang['home'] = 'Home';
$cs_lang['messages'] = 'Mensajes';
$cs_lang['logout'] = 'Desconectarse';
$cs_lang['settings'] = 'Ajustes';
$cs_lang['register'] = 'Registración';
$cs_lang['sendpw'] = 'Perdio la Contrase&ntilde;a ?';
$cs_lang['system'] = 'Sistema';
$cs_lang['panel'] = 'Administración';

// Manage
$cs_lang['new_user']	= 'Crear Nuevo Usuario';
$cs_lang['access']	= 'Acceso';
$cs_lang['country']	= 'Pais';
$cs_lang['nick']	= 'Nick';
$cs_lang['laston']	= 'Ultima Visita';
$cs_lang['page']	= 'Status';
$cs_lang['active']	= 'Activado';
$cs_lang['picture_edit'] = 'Editar la imagen del Perfil';
$cs_lang['search'] = 'Search';

// View
$cs_lang['profile'] = 'Perfil';
$cs_lang['person'] = 'Persona';
$cs_lang['phone'] = 'Telefono';
$cs_lang['postal_place'] = 'Codigo Postal - Residencia';
$cs_lang['registered'] = 'Registración';
$cs_lang['sex'] = 'Sexo';
$cs_lang['surname'] = 'Apellido';
$cs_lang['url'] = 'Pagina Web';
$cs_lang['buddy_add'] = 'A&ntidle;adir como Amigo';
$cs_lang['prename'] = 'Nombre';
$cs_lang['birth_age'] = 'Fecha de Nacimiento (Edad)';
$cs_lang['nopic'] = 'No existe imagen';
$cs_lang['height'] = 'Altura';
$cs_lang['adress'] = 'Calle';
$cs_lang['edit_profile'] = 'Editar Perfil';
$cs_lang['edit_picture'] = 'Editar Imagen';
$cs_lang['contact'] = 'Contato';
$cs_lang['email'] = 'E-Mail';
$cs_lang['icq'] = 'ICQ';
$cs_lang['mobile'] = 'Mobil';
$cs_lang['msn'] = 'MSN';
$cs_lang['skype'] = 'Skype';
$cs_lang['info'] = 'Info';
$cs_lang['message_send'] = 'Enviar Mensaje';

// Create
$cs_lang['create_users']	= 'A&ntilde;adir nuevas Cuentas de Usuarios';
$cs_lang['lang'] = 'Lengua';
$cs_lang['password'] = 'Contrase&ntilde;a';
$cs_lang['extended'] = 'Avanzado';
$cs_lang['send_mail'] = ' Enviar Datos a su E-Mail';
$cs_lang['reg_done'] = 'Operacion Valida';

$cs_lang['short_nick'] = '- El Nick es muy corto (min. %d Letras).';
$cs_lang['nick_exists'] = '- El Nick existe, por Favor escoja uno nuevo';
$cs_lang['short_pwd'] = '- La Contrase&ntilde;a es muy corta (min. 4 Letras).';
$cs_lang['email_exists'] = '- La E-Mail-Adresse esta en uso';
$cs_lang['email_false'] = '- La E-Mail no es valida';
$cs_lang['no_access'] = '- Tiene que escojer un Acceso';
$cs_lang['access_denied'] = '- El Acceso es mal alto que el Propio';

$cs_lang['auto_pass']	= '- La Contrase&ntilde;a esta siendo creada';

// Mail
$cs_lang['mail_reg_head'] = 'Registracion';
$cs_lang['mail_reg_start'] = "\n Bienvenido a la comunidad, ";
$cs_lang['mail_reg_start'] .= "con estos datos podra usar el login : \n";
$cs_lang['mail_reg_nick'] = "\n Nick : ";
$cs_lang['mail_reg_password'] = "\n Contraseña : ";
$cs_lang['mail_reg_password2'] = "Usetd ya ha creado su Contraseña, al rellenar el Formulario de Join Us";
$cs_lang['mail_reg_ip'] = "\n \n Petición de la IP ";
$cs_lang['mail_reg_ask'] = "\n \n Para Preguntas por favor und Email a ";
$cs_lang['mail_reg_end'] = "\n \n Esta Email ha sido pedida por un Formulario.";



// Edit
$cs_lang['state'] = 'Status';
$cs_lang['not_active'] = 'Deactivado';
$cs_lang['not_active_text'] = 'El Usuario ha sido Deactivado';

// Logout
$cs_lang['logged_out']	= 'Desconexion valida, Hasta Pronto !';

// Login
$cs_lang['login'] = 'Connectar';
$cs_lang['login_messages']	= 'Aqui salen todas las Noticias despues del envio.';
$cs_lang['pwd'] = 'Contrase&ntilde;a';
$cs_lang['user_notfound'] = 'Este Usuario no existe el el Banco de Datos';
$cs_lang['wrong_pwd'] = 'La Contrase&ntilde;a no es valida';
$cs_lang['closed'] = 'El Usuario ha sido Deactivado';
$cs_lang['method_form'] = 'Coneccion Valida';
$cs_lang['method_form_cookie'] = 'La Coneccion ha sido Valida y el Cookie ha sido aceptado';
$cs_lang['method_cookie'] = 'Su Login ha sido reactivado por Cookies.';
$cs_lang['method_session'] = 'Ya existe una Conección activa';
$cs_lang['not_public'] = 'La Coneccion en el Modo de mantenimiento requiere Level de Acceso 5';

// Register
$cs_lang['signup'] = 'Registrar';
$cs_lang['register_info']	= 'Aqui se puede Crear una cuenta de Uso';
$cs_lang['register_disabled'] = 'La Registraciòn ha sido deactivado por el Administrador';
$cs_lang['mail_key'] = 'Link de Activación';

// Home
$cs_lang['home_info']	= 'Todas Informaciones sobre la cuenta de Uso';

// Settings
$cs_lang['settings_info']	= 'Todas Informaciones sobre la cuenta de Uso';
$cs_lang['picture']	= 'Imagen';
$cs_lang['setup']	= 'Ajustes del Sistema';
$cs_lang['close']	= 'Deactivar su Cuenta';

// Profile
$cs_lang['column'] = 'Columna';
$cs_lang['value'] = 'Valor';
$cs_lang['hidden'] = 'Esconder';
$cs_lang['birthday'] = 'Cumplea&ntilde;os';

// Picture
$cs_lang['picture_manage']	= 'Hier wird das Benutzerbild verwaltet.';
$cs_lang['current']	= 'Actual';
$cs_lang['pic_infos'] = 'Infobox';
$cs_lang['max_width'] = 'Anchura Maximal : ';
$cs_lang['max_height'] = 'Altura Maximal : ';
$cs_lang['max_size'] = 'Capacidad Maximal :  ';
$cs_lang['filetypes'] = 'Formados de Archivos : ';

$cs_lang['too_big']	= '- El Archivo es muy grande';
$cs_lang['too_wide']	= '- La imagen es muy ancha ';
$cs_lang['too_high']	= '- La imagen es muy alta ';
$cs_lang['ext_error']	= '- Se ha usado un Formado de archivos no Permitido';
$cs_lang['up_error'] = '- A ocurrido un error desonocido, po Favor contacte el Administrador de la Pagina';

// Setup
$cs_lang['timezone'] = 'Zona de la Hora';
$cs_lang['dstime'] = 'Hora de Verano';
$cs_lang['limit'] = 'Datos por Pagina';
$cs_lang['view'] = 'Vista';
$cs_lang['off'] = 'desactivado';
$cs_lang['on'] = 'activado';
$cs_lang['checked'] = 'checked';
$cs_lang['homelimit'] = 'Temas en La Pagina Basica';
$cs_lang['categories'] = 'Categorias';

// Password
$cs_lang['change_password']	= 'Aqui puede editar la Contrase&ntilde;a';
$cs_lang['pwd_current'] = 'Aktuelles Contrase&ntilde;a';
$cs_lang['pwd_new'] = 'Nueva Contrase&ntilde;a';
$cs_lang['pwd_again'] = 'Repetir Contrase&ntilde;a';
$cs_lang['wrong_current_pwd'] = '- La Contrase&ntilde;a actual no es correcta';
$cs_lang['not_match'] = '- Las nuevas Contrase&ntilde;as no son Identicas ';

// Close
$cs_lang['close_confirm'] = 'Quiere Desactivar de verdad su cuenta de Usuario ?';
$cs_lang['close_info'] = 'Solo sera poder activado de nuevo pr el Administrador!';
$cs_lang['close_false']	= 'Desactivación Interrumpida';
$cs_lang['close_true']	= 'Desactivación Validada';

// List
$cs_lang['list']	= 'Lista';
$cs_lang['female']	= 'feminino';
$cs_lang['male']		= 'masculino';
$cs_lang['place']		= 'Residencia';

// Options
$cs_lang['manage_options']	= 'Administración de las Opciones del Modul ';
$cs_lang['pixel'] = 'Pixel';
$cs_lang['bytes'] = 'Bytes';
$cs_lang['letters'] = 'Letras';
$cs_lang['min_letters'] = 'Minimo de Letras del Nick';
$cs_lang['def_register'] = 'Modo de Registración';
$cs_lang['reg_mail'] = 'Validacio por E-Mail';
$cs_lang['reg_captcha'] = 'Inserar Codigo de Seguridad';

// SendPW
$cs_lang['sendpw_info']	= 'Escriba la E-mail que pertenece a su cuenta de usuario';
$cs_lang['sendpw_info'] .= 'Sies valida adquirira una clave para poder ';
$cs_lang['sendpw_info'] .= 'activar la nueva Contrase&ntilde;a';
$cs_lang['request'] = 'Peticiones';

$cs_lang['email_unknown'] = '- La email no se ha podido encontrar';
$cs_lang['email_found'] = 'Escriba la clave que ha sido enviada a su e-Mail para poder activar ';
$cs_lang['email_found'] .= 'una nueva Contrase&ntilde;a para su cuenta';
$cs_lang['error_key'] = '- La clave no es correcta';
$cs_lang['sendpw_done'] = 'La Contrase&ntilde;a a sido validada y cambiada';

$cs_lang['mail_spw_head'] = "Perdio la Contrase&ntilde;a ?";
$cs_lang['mail_spw_start'] = "En la Pagina http://";
$cs_lang['mail_spw_start2'] = " hubo Petición para una nueva Contrase&ntilde;a.";
$cs_lang['mail_spw_start3'] = "\n\nAqui los Datos : ";
$cs_lang['mail_spw_key'] = "\n\nClave : ";
$cs_lang['mail_spw_ip'] = "\nPetición de la  IP : ";
$cs_lang['mail_spw_ask'] = "\n\nPreguntas pro favor via per E-Mail a ";
$cs_lang['mail_spw_end'] = "\n\nEsta E-Mail fue pedeida pro un Formulario";

$cs_lang['key'] = 'Clave';
$cs_lang['new_pwd'] = 'Contrase&ntilde;a';

$cs_lang['online']	= 'Online';
$cs_lang['all']	= 'Todo el Mundo';
$cs_lang['count_users'] 	= 'Se encuentran %s usuarios en la Pagina';

$cs_lang['avatar'] = 'Imagen del Foro';
$cs_lang['signature'] = 'Signatur';

$cs_lang['mail_key_link'] = '<a href="http://';
$cs_lang['mail_key_link2'] = '?mod=users&action=activate&key=';
$cs_lang['mail_key_email'] ='&email=';
$cs_lang['mail_key2'] = '">Link de Activación</a>';
$cs_lang['mail_no_link'] = '\n\nSi no ve el Link correctamente copie el codigo y inserero en su Browser';
$cs_lang['key_mail_false'] = '- Clave o E-Mail no Valida';
$cs_lang['already_activated'] = '- Cuenta lla esta activada';
$cs_lang['account_activated'] = 'Activación de la cuenta Valida';
$cs_lang['activate_acc'] = 'Activar Cuenta';
$cs_lang['no_activation'] = 'Cuenta ya Activa o Clave no valida';


$cs_lang['del_true'] = 'El Usuario ha sido extraido del Banco de Datos';

$cs_lang['default'] = 'Standard';


// Nivel de Seguridad
$cs_lang['secure'] = 'Nivel de Seguridad :';  
$cs_lang['secure_stages'] = '<b>Niveles de Seguridad :</b>';  
$cs_lang['pw_infos'] = 'Información:';  
$cs_lang['stage_1'] = 'Nivel 1';  
$cs_lang['stage_2'] = 'Nivel 2';  
$cs_lang['stage_3'] = 'Nivel 3';  
$cs_lang['stage_4'] = 'Nivel 4';  
$cs_lang['stage_1_text'] = ': min. 8 Letras.';  
$cs_lang['stage_2_text'] = ': min. 8 Letras de estas 2 Numeros.';  
$cs_lang['stage_3_text'] = ': min. 8 Letras, 3 Letras peque&ntilde;as, 2 Letras Grandes & 2 Numeros.';  
$cs_lang['stage_4_text'] = ': min. 8 Letras, 3 Letras peque&ntilde;as, 2 Letras Grandes, 2 Numeros & 1 Carácter especial';

// Navrand
$cs_lang['since'] = 'Registered';
$cs_lang['noname'] = 'No Name';
?>