<?php
// Modul Nombre : Translation in Spanish
// CS Version : ClanSphere 2008 - SP1
// Team :  quemao-designz.de - www.quemao-designz.de
// Author : quema0
// Author eMail : quemao@quemao-designz.de
// $Id$
$cs_lang['mod_name']  = 'Paginas Staticas';
$cs_lang['mod_text']  = 'Administración';
$cs_lang['mod']  = 'Paginas Staticas';
$cs_lang['head_create'] = 'Crear';
$cs_lang['body_create'] = 'Por favor rellene las casillas con *';
$cs_lang['head_edit'] = 'Editar';
$cs_lang['body_edit'] = 'Por favor rellene las casillas con *';
$cs_lang['body_com_edit']  = 'Por favor Informar sobre editaciones del Texto';
$cs_lang['head_com_edit']  = 'Editar comentarios';
$cs_lang['head_com_remove']  = 'Remover Comentarios';
$cs_lang['head_remove']  = 'Remover';
$cs_lang['title'] = 'Titulo';
$cs_lang['access'] = 'Acceso';
$cs_lang['content'] = 'Contenido';
$cs_lang['config'] = 'Ajustes';
$cs_lang['tablelayout'] = 'Layout de Tablas';
$cs_lang['activate_comments'] = 'Permitir Comentarios';
$cs_lang['admins_only'] = 'Usar PHP-Code (Webmaster only)';
$cs_lang['no_access'] = 'Acceso Bajo';
$cs_lang['new_staticpage'] = 'Nueva Pagina';
$cs_lang['id'] = '#ID';
$cs_lang['all'] = 'Todo el Mundo';
$cs_lang['no_title'] = '- Falta el Titulo';
$cs_lang['edit_done'] = 'Editacion Valida';
$cs_lang['lev_0'] = 'Todo el Mundo';
$cs_lang['lev_1'] = 'Visitante =>';
$cs_lang['lev_2'] = 'Usuario =>';
$cs_lang['lev_3'] = 'Miembros =>';
$cs_lang['lev_4'] = 'Moderatores =>';
$cs_lang['lev_5'] = 'Administradores';
$cs_lang['not_able']  = '- Comentarios han sido desativados';
$cs_lang['place']      = 'Lugar :';
$cs_lang['posts']      = 'Aportaciones:';
$cs_lang['text']    = 'Texto';
?>