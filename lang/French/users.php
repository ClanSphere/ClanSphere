<?php
// ClanSphere 2006 - www.clansphere.net - Traduction francaise >>> www.globalteam-esports.info
// $Id$

$cs_lang['mod_name']  = 'Utilisateurs';
$cs_lang['users'] = 'Utilisateurs';
$cs_lang['mod_text']  = 'Administration des comptes';

$cs_lang['wizard'] = 'Assistant';
$cs_lang['task_done'] = 'Marquer la tâche effectuée';

// Navlist
$cs_lang['admin'] = 'Administration';
$cs_lang['cookie'] = 'enregistrer un cookie';
$cs_lang['home'] = 'Accueil personnel';
$cs_lang['messages'] = 'Messages';
$cs_lang['logout'] = 'Déconnexion';
$cs_lang['settings'] = 'Réglage personnel';
$cs_lang['register'] = 'Enregistrement';
$cs_lang['sendpw'] = 'Mot de passe oublié';
$cs_lang['system'] = 'Système';
$cs_lang['panel'] = 'Administration';

// Manage
$cs_lang['new_user']  = 'Nouvel utilisateur';
$cs_lang['access']  = 'Accès';
$cs_lang['country']  = 'Pays';
$cs_lang['nick']  = 'Nick';
$cs_lang['laston']  = 'Dernière visite';
$cs_lang['page']  = 'En ligne';
$cs_lang['active']  = 'Activé';
$cs_lang['picture_edit'] = 'Travailler l\'image du profil';
$cs_lang['search'] = 'Search';

// View
$cs_lang['profile'] = 'Profile';
$cs_lang['person'] = 'Personne';
$cs_lang['phone'] = 'Téléphone';
$cs_lang['postal_place'] = 'Code postal - Lieu';
$cs_lang['registered'] = 'Enregistré';
$cs_lang['sex'] = 'Sexe';
$cs_lang['surname'] = 'Nom';
$cs_lang['url'] = 'Page d\'accueil';
$cs_lang['buddy_add'] = 'Ajouter un ami';
$cs_lang['prename'] = 'Prénom';
$cs_lang['birth_age'] = 'Anniversaire (age)';
$cs_lang['nopic'] = 'Image introuvable';
$cs_lang['height'] = 'Taille';
$cs_lang['adress'] = 'Adresse';
$cs_lang['edit_picture'] = 'Editer l\'image';
$cs_lang['edit_profile'] = 'Editer le profile';

$cs_lang['contact'] = 'Contact';
$cs_lang['email'] = 'E-Mail';
$cs_lang['icq'] = 'ICQ';
$cs_lang['mobile'] = 'Téléphone portable';
$cs_lang['msn'] = 'MSN';
$cs_lang['skype'] = 'Skype';
$cs_lang['info'] = 'Info';
$cs_lang['message_send'] = 'Envoyer un message';

// Create
$cs_lang['create_users']  = 'Ajouter un nouveau compte d\'utilisateur.';
$cs_lang['lang'] = 'Langage';
$cs_lang['password'] = 'Mot de passe';
$cs_lang['extended'] = 'Plus';
$cs_lang['send_mail'] = ' Envoyer un E-Mail avec les données';
$cs_lang['reg_done'] = 'Utilisateur ajouté avec succès.';

$cs_lang['short_nick'] = '- Le nick est trop court (min. 4 lettres)';
$cs_lang['nick_exists'] = '- Le nick est déjà alloué';
$cs_lang['short_pwd'] = '- Le mot de passe est trop court (min. 4 lettres)';
$cs_lang['email_exists'] = '- Cette adresse E-Mail est déjà allouée';
$cs_lang['email_false'] = '- E_Mail invalide';
$cs_lang['no_access'] = '- Saisissez un accès';
$cs_lang['access_denied'] = '- Droit insuffisant';

$cs_lang['auto_pass']  = 'Le mot de passe est créé';

// Mail
$cs_lang['mail_reg_head'] = 'Enregistrement';
$cs_lang['mail_reg_start'] = "\n Bienvenue dans la communauté, ";
$cs_lang['mail_reg_start'] .= "Vous pouvez vous loggez avec les données suivantes: \n";
$cs_lang['mail_reg_nick'] = "\n Nick: ";
$cs_lang['mail_reg_password'] = "\n Mot de passe: ";
$cs_lang['mail_reg_password2'] = "Tu as déjà fourni ce mot de passe dans joinus";
$cs_lang['mail_reg_ip'] = "\n \n Demande de l'IP ";
$cs_lang['mail_reg_ask'] = "\n \n Questions par émail ";
$cs_lang['mail_reg_end'] = "\n \n Email demandée par formulaire.";

// Edit
$cs_lang['state'] = 'Etat';
$cs_lang['not_active'] = 'Déactivé';
$cs_lang['not_active_text'] = 'L\'utilisateur choisi n\'est plus actif';

// Logout
$cs_lang['logged_out']  = 'Fin du processus, à bientôt!';

// Login
$cs_lang['login'] = 'Login';
$cs_lang['login_messages']  = 'Tous les messages apparaîssent ici après avoir posté.';
$cs_lang['pwd'] = 'Mot de passe';
$cs_lang['user_notfound'] = 'Cet utilisateur n\'existe pas dans la banque de donnée.';
$cs_lang['wrong_pwd'] = 'Mot de passe incorrect.';
$cs_lang['closed'] = 'L\'utilisateur est désactivé.';
$cs_lang['not_public'] = 'Le Login de maintenance nécessite le droit d\'administrateur.';
$cs_lang['method_form'] = 'Connexion avec succès.';
$cs_lang['method_form_cookie'] = 'Connexion avec succès, le cookie a été mis en place.';
$cs_lang['method_cookie'] = 'Votre login vient d\'être réactivé à l\'aide du cookie.';
$cs_lang['method_session'] = 'Vous avez déjà une session active.';

// Register
$cs_lang['signup'] = 'S\'inscrire';
$cs_lang['register_info']  = 'Vous pouvez vous inscrire ici.';
$cs_lang['register_disabled'] = 'L\'enregistrement d\'utilisateur vient d\'être bloqué par l\'administrateur.';

$cs_lang['mail_key'] = 'Lien d\'activation';

// Home
$cs_lang['home_info']  = 'Compte d\'utilisateur en un coup d\'oeil.';

// Settings
$cs_lang['settings_info']  = 'Tout sur le compte d\'utilisateur.';
$cs_lang['picture']  = 'Image';
$cs_lang['setup']  = 'Réglage';
$cs_lang['close']  = 'Désactiver';

// Profile
$cs_lang['column'] = 'Colonne';
$cs_lang['value'] = 'Valeur';
$cs_lang['hidden'] = 'Cacher';
$cs_lang['birthday'] = 'Anniversaire';

// Picture
$cs_lang['picture_manage']  = 'Administration de l\'image d\'utilisateur.';
$cs_lang['current']  = 'Récent';
$cs_lang['pic_infos'] = 'Aide';
$cs_lang['max_width'] = 'Largeur maximale :';
$cs_lang['max_height'] = 'Hauteur maximale :';
$cs_lang['max_size'] = 'Taille maximale :';
$cs_lang['filetypes'] = 'Format du fichier :';

$cs_lang['too_big']  = '- Taille du fichier trop importante';
$cs_lang['too_wide']  = '- L\'image est trop large';
$cs_lang['too_high']  = '- L\'image est trop grande';
$cs_lang['ext_error']  = '- Format de fichier non autorisé';
$cs_lang['up_error'] = '- Veuillez contacter l\'administrateur du site';

// Setup
$cs_lang['timezone'] = 'Fuseau horaire';
$cs_lang['dstime'] = 'Heure d\'été';
$cs_lang['limit'] = 'Nombre de bloc de données par page';
$cs_lang['view'] = 'Vue';
$cs_lang['automatic'] = 'automatique';
$cs_lang['off'] = 'Arrêt';
$cs_lang['on'] = 'Marche';
$cs_lang['checked'] = 'Vérifié';
$cs_lang['homelimit'] = 'Ensemble des fils d\'accueil';
$cs_lang['readtime'] = 'Cochez les sujets à lire';
$cs_lang['days'] = 'Jours';
$cs_lang['no_readtime'] = 'Saisissez le nombre de jour';
$cs_lang['default'] = 'Standard';
$cs_lang['categories'] = 'Catégories';
// Password
$cs_lang['change_password']  = 'Vous pouvez changer ici votre mot de passe .';
$cs_lang['pwd_current'] = 'Mot de passe actuel';
$cs_lang['pwd_new'] = 'Nouveau mot de passe';
$cs_lang['pwd_again'] = 'Répéter le mot de passe ';
$cs_lang['wrong_current_pwd'] = '- Le mot de passe actuel est incorrect';
$cs_lang['not_match'] = '- Les nouveaux mots de passe ne sont pas identiques';

// Close
$cs_lang['close_confirm'] = 'Voulez vous vraiment fermer votre compte?';
$cs_lang['close_info'] = 'Ce compte ne peut être réouvert qu\'avec l\'aide de l\'administrateur!';
$cs_lang['close_false']  = 'Fermeture annulée';
$cs_lang['close_true']  = 'Fermeture réussie';

// List
$cs_lang['list']  = 'Liste';
$cs_lang['female']  = 'Féminin';
$cs_lang['male']    = 'Masculin';
$cs_lang['place']    = 'Lieu';

// Options
$cs_lang['manage_options']  = 'Administration des réglages dans le module.';
$cs_lang['pixel'] = 'Pixels';
$cs_lang['bytes'] = 'Octets';
$cs_lang['letters'] = 'Caractères';
$cs_lang['min_letters'] = 'Longueur minimale du pseudo';
$cs_lang['def_register'] = 'Mode d\'enregistremenet';
$cs_lang['reg_mail'] = 'Validation de l\'E-Mail';
$cs_lang['reg_captcha'] = 'Saisie du code de sécurité';

// SendPW
$cs_lang['sendpw_info']  = 'Saisissez votre Email. ';
$cs_lang['sendpw_info'] .= 'Si votre mail est correcte, vous recevrez votre clef ';
$cs_lang['sendpw_info'] .= 'pour activer le nouveau mot de passe.';
$cs_lang['request'] = 'Demander';

$cs_lang['email_unknown'] = '- E-Mail invalide.';
$cs_lang['email_found'] = 'Inscrivez les chiffres de la clef que vous avez reçu par E-Mail';
$cs_lang['email_found'] .= 'Nouveau mot de passe pour votre compte.';
$cs_lang['error_key'] = '- Clef invalide';
$cs_lang['sendpw_done'] = 'Changement de mot de passe réussi.';
$cs_lang['mail_spw_start2'] = " demande de mot de passe.";
$cs_lang['mail_spw_start3'] = "\n\ninformations demandée: ";
$cs_lang['key'] = 'Clef';
$cs_lang['new_pwd'] = 'Mot de passe';
$cs_lang['mail_spw_start'] = "\n suivant les données demandées: ";
$cs_lang['mail_spw_key'] = "\n \n clé: ";
$cs_lang['mail_spw_ip'] = "\n \n demandé de l\'IP ";
$cs_lang['mail_spw_ask'] = "\n \n demande de question par Email ";
$cs_lang['mail_spw_end'] = "\n \n Cet Email a été généré automatiquement.";
$cs_lang['mail_spw_head'] = 'Mot de passe oublié';

$cs_lang['online']  = 'Online';
$cs_lang['all']  = 'Tous';
$cs_lang['count_users']   = '%s utilisateur(s)sur cette page';

$cs_lang['avatar'] = 'Avatar';
$cs_lang['signature'] = 'Signature';

$cs_lang['mail_key_link'] = '<a href="http://';
$cs_lang['mail_key_link2'] = '?mod=users&action=activate&key=';
$cs_lang['mail_key_email'] ='&email=';
$cs_lang['mail_key2'] = '">Lien d\'activation</a>';
$cs_lang['mail_no_link'] = '\n\n Si le lien ne fonctionne pas à partir du mail, veuillez le copier dans votre navigateur.';
$cs_lang['key_mail_false'] = '- Clé ou Email invalide';
$cs_lang['already_activated'] = '- Compte déjà réactivé';
$cs_lang['account_activated'] = 'Compte réactivé avec succès';
$cs_lang['activate_acc'] = 'Activer le compte';
$cs_lang['no_activation'] = 'Le compte est réactivé ou bien la clef est invalide';

// Password Check
$cs_lang['secure'] = 'Security Check:';
$cs_lang['secure_stages'] = '<b>Security Stages:</b>';
$cs_lang['del_true'] = 'Utilisateur supprimé avec succès.';

$cs_lang['pw_infos'] = 'Information:';
$cs_lang['stage_1'] = 'Stage 1';
$cs_lang['stage_2'] = 'Stage 2';
$cs_lang['stage_3'] = 'Stage 3';
$cs_lang['stage_4'] = 'Stage 4';
$cs_lang['stage_1_text'] = ': min. 8 Character.';
$cs_lang['stage_2_text'] = ': mind. 8 Character with 2 Numbers.';
$cs_lang['stage_3_text'] = ': mind. 8 Character, 3 small Letters, 2 large Letters & 2 Numbers.';
$cs_lang['stage_4_text'] = ': mind. 8 Character, 3 small Letters, 2 large Letters, 2 Numbers & 1 Special character';

// Navrand
$cs_lang['since'] = 'Registered';
$cs_lang['noname'] = 'No Name';
?>