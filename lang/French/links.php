<?php
// ClanSphere 2006 - www.clansphere.net - Traduction francaise >>> www.globalteam-esports.info
// $Id$

$cs_lang['mod']  = 'Liens';
$cs_lang['text']  = 'Le module de lien';

$cs_lang['head']= 'Aperçu';
$cs_lang['body_list']= '%s lien(s)enregistré(s).';

$cs_lang['all'] = 'Total :';
$cs_lang['url'] = 'URL';
$cs_lang['cat'] = 'Catégorie';
$cs_lang['stat'] = 'Status';
$cs_lang['new_link'] = 'Nouveau lien';


$cs_lang['online'] = '[color=lime]en ligne[/color]';
$cs_lang['offline'] = '[color=red]hors ligne[/color]';

$cs_lang['head_remove']  = 'Supprimer';
$cs_lang['newicon'] = 'Nouvelle bannière';
$cs_lang['delicon'] = 'Supprimer la bannière';

$cs_lang['mod1']  = 'Sponsors et partenaires';
$cs_lang['all1'] = '%s sponsors et partenaires enregistrés jusqu\'à présent.';

$cs_lang['info'] = 'Infos';
$cs_lang['head1'] = 'Détails';   

$cs_lang['head2'] = 'Créer';
$cs_lang['body_create'] =  'Veuillez remplir les champs marqués d\'une étoile.';
$cs_lang['error_occurred'] = 'Remplissez correctement les champs.';
$cs_lang['error_count'] = 'Les champs ne sont pas correctement remplis.';

$cs_lang['categories'] = 'Catégorie';
$cs_lang['status'] = 'Status';
$cs_lang['icon'] = 'Bannière'; 

$cs_lang['extension'] = 'Etendre';
$cs_lang['sponsorbox'] = 'Enregistrer comme sponsors ou partenaire';

$cs_lang['ext_error'] ='- Fichier non-autorisé';
$cs_lang['too_wide'] ='- Bannière trop large';
$cs_lang['too_high'] ='- Bannière trop haute';
$cs_lang['too_big'] ='- Taille du fichier trop importante';

$cs_lang['pic_infos'] = 'Aide';
$cs_lang['max_width'] = 'Largeur maximale :';
$cs_lang['max_height'] = 'Hauteur maximale :';
$cs_lang['max_size'] = 'Taille maximale :';
$cs_lang['filetypes'] = 'Format du fichier :';

$cs_lang['cat_manage'] = 'Administration';
$cs_lang['cat_create'] = 'Nouveau';

$cs_lang['online1'] = 'en ligne';
$cs_lang['offline1'] = 'hors ligne';

$cs_lang['success_noup'] = 'Lien enregistré avec succès, cependant une erreur est survenue lors du téléchargent de l\'image.';

?>