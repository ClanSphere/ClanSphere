...
<table class="forum" cellpadding="0" cellspacing="{page:cellspacing}" style="width:{page:width}">
  <tr>
    <td class="headb" colspan="3">{lang:visitors} - {lang:yearoverview} {head:year}</td>
  </tr>
</table>
<br />
<div id="flashcontent">
...
<script type="text/javascript">
  ...
  so.addVariable("chart_data", 
                 "<chart>" + 
                   "<series>" +
                     {loop:count}"<value xid='{count:id}'>{count:month}<\/value>" + {stop:count}
                   "<\/series>" + 
                   "<graphs>" + 
                     "<graph gid='1' title='{lang:guest}'>" +
                       {loop:countall}"<value xid='{countall:id}'>{countall:value}<\/value>" + {stop:countall}
                     "<\/graph>" +
                     "<graph gid='2' title='{lang:daydif}'>" +
                       {loop:countave}"<value xid='{countave:id}'>{countave:value}<\/value>" + {stop:countave}
                     "<\/graph>" +
                   "<\/graphs>" +
                 "<\/chart>");
